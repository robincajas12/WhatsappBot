...
export class RemoveAiCommand implements Command {
    ...
    public async execute(message: WAMessage, sock: ReturnType<typeof makeWASocket>, args: string[]): Promise<void> {
        ...
        if (subCommand === 'ai' && targetUser) {
            ...
            RemoveAiCommand.dbService.removeUser(targetJid);
            await sock.sendMessage(sender!, { text: `Usuario ${targetJid} eliminado de la lista de IA.` });
        } else {
            await sock.sendMessage(sender!, { text: `Formato incorrecto. Usa: !remove ai <número>` });
        }
    }
}
